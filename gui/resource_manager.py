...
@dataclass
class SaveItem:
    type: Literal['mask', 'visualization', 'soft_mask', 'depth']
...
class ResourceManager:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        if self.workspace is None:
            ...
            self.workspace = path.join(cfg.get('workspace_root', './workspace'), basename)
        ...
        self.depth_dir = path.join(self.workspace, 'depth')
    ...
    def shutdown(self):
        """Stop the save threads, flushing anything still queued. Idempotent.

        Call this before dropping a ResourceManager (e.g. when swapping workspaces in the
        running GUI): the LRU image/mask caches hold bound methods of this instance, so the
        object sits in a reference cycle and __del__ would not run promptly on its own --
        without an explicit stop the save threads would leak on every switch."""
    ...
    def save_thread(self, queue: Queue):
        while True:
            ...
            if args.type == 'mask':
                ...
                # float [0,1] H*W -> 8-bit grayscale depth map (like masks, one png/frame)
                cv2.imwrite(path.join(self.depth_dir, args.name + '.png'),
                            (args.data * 255).astype(np.uint8))
    ...
    def save_depth(self, ti: int, depth: np.ndarray):
        # depth should be float [0,1] H*W
        assert 0 <= ti < self.length
        assert isinstance(depth, np.ndarray)

        self.add_to_queue_with_warning(SaveItem('depth', depth, self.names[ti]))
