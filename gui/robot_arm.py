...
straight rigid shaft, so the tracker may slide a side across the arm and along it, but it
may not pivot one. A pixel of tracker drift over a long arm is a degree of pivot the arm
never made, and that degree lands straight on the reading -- the width below is read
SQUARE to the axis these four points define, and a chord read oblique to a cylinder comes
out too wide, which is the one error this whole tool is built to avoid. A frame you
correct by hand is a keyframe: the tracker re-seeds there -- taking the slope you just
drew as the new one -- and TRACK never overwrites it. The run STOPS the moment an entry point moves further in one frame than
...
# made, and that degree is scale error: snap_width reads the mask's chord SQUARE to the
# axis these points define, and a chord read oblique to a cylinder is too wide.
# Holding the slope costs nothing that is real -- where each side sits across the arm and
# how far along the tip is are still the tracker's to say, and the width was never its to
# say in the first place. That is all a rigid arm can actually do on the screen.
...
if __name__ == '__main__':
    ...
    along = lock.moved_to([lock.pts[0], lock.pts[1]]
                         + [(lock.pts[2 + i][0] + 20.0 * dirs[i][0],
                             lock.pts[2 + i][1] + 20.0 * dirs[i][1]) for i in (0, 1)],
                         shape=shape, dirs=dirs)
    assert abs(along.diameter - lock.diameter) < 0.1, along.diameter  # a tenth of a pixel:
    ...
    assert along.length_px > lock.length_px + 15.0
